...
const genFieldFromString = function genFieldFromString(key): $Exact<$fieldProperties> {
...
function genFieldConfig(fields, key, initialValue) {
  ...
  return { ...genFieldFromString(key), ...field, isValid: !!initialValue, value: initialValue };
}
...
export function form<$props: *, $fieldsConfig: *, $actionsConfig: *, $config: *>(
  ...
  return (WrappedComponent: *)=>class Form extends PureComponent {
    state: {
			fields: $fields,
			isValid: boolean;
		};
    ...
    componentWillReceiveProps(nextProps: $props) {
      ...
      if (get(nextProps, nextConfig.reset || '') !== get(this.props, nextConfig.reset || '')) {
      ...
      } else if (initialPropName && nextProps[initialPropName] !== this.props[initialPropName]) {
        this.reinitialize(nextProps);
      }
    }
    ...
    getInitialState = (props: $props) => {
      ...
      const fieldsMap = Object.keys(fields).reduce((allFields, key) => {
        const fieldConfig = genFieldConfig(fields, key, initialValues[key] || get(props, `${config.initialValuesPropName || ''}.${key}`));
        return allFields.set(
          ...
          new Field({
            ...
            interact: fieldConfig.interact && interact ? { ...interact, entity: `${interact.context}_${key.toUpperCase()}` } : false,
          }),
        );
      }, new Map());
      const initialState = {
      ...
      this.entities = {
        submit: {
          ...
          children: 'Submit',
          styleType: 'primary',
          interact: interact ? { ...interact, entity: `${interact.context}_SUBMIT` } : false,
        },
        cancel: {
          onClick: this.cancel,
          children: 'Cancel',
          styleType: 'default',
          interact: interact ? { ...interact, entity: `${interact.context}_CANCEL}` } : false,
        },
      };
      return initialState;
    }
    ...
    reinitialize = (props: $props) => {
      ...
      this.setState((prevState) => {
        const fields = prevState.fields.map((field) => {
          ...
          const value = initialValues[name] || get(props, `${config.initialValuesPropName || ''}.${name}`);
          if (isDirty) {
            return field;
          }
          return field.merge({
    ...
    isValid = (fields: Map<string, Field>)=>fields.toKeyedSeq().reduce((valid, nextField) => {
      if (valid) {
    ...
    onChange({ name, value }: {name: string; value: *}) {
      ...
      this.setState((previousState) => {
        ...
        if (updateCallback) {
          setTimeout(() => {
            const currentTime = new Date().getTime() - (config.debounce || 500);
            if (this.startTime <= currentTime || (this.lastKey !== name && this.tokensByKey[name] === token)) {
              updateCallback(newField, this.jsonifyFields()).catch(this.handleErrors);
            }
          }, 500);
        }
    ...
    submit() {
      ...
      return submit(this.jsonifyFields())
    ...
    handleErrors(errors: Object) {
      const nextFields = Object.keys(errors).reduce((fields, key) => fields.updateIn([key, 'errors'], (errorsList) => {
        if (errorsList) {
          return errorsList.concat(errors[key]);
        }
        return new List();
      })
				.setIn([key, 'isValid'], false), this.state.fields);
