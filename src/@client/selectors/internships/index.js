...
class InternshipSelectors extends Selector {
  ...
  getProjectId = (internshipIdSelector) => {
    return this.findRelatedId('project', internshipIdSelector || defaultIdSelector);
  }
  getUserId = (internshipIdSelector) => projectSelectors.findRelatedId('user', this.getProjectId(internshipIdSelector))
  canEdit = (internshipIdSelector)=>createSelector([
    this.getUserId(internshipIdSelector),
    sessionSelectors.getCurrentUserId(),
  ], (userId, currentUserId)=>{
    return Number(userId) === Number(currentUserId);
  })
  internExists = (internshipIdSelector)=>createSelector([
