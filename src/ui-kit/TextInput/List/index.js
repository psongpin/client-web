...
export default class TextInputList extends PureComponent {
  ...
  handleCreate = (value: *) => {
    return this.props.onCreate(value).then(this.createSuccessful);
  };
  createSuccessful = () => {
    this.setState({
      createKey: Math.random(),
          ...
          return (
            <ULItem
              rightActions={[
                <Icon
                  name="trash"
                  onClick={() => this.props.onRemove(entity.id)}
                />,
              ]}
            >
              {
                // $FlowFixMe
                <Comp
                  value={entity[this.props.valueProp || 'content']}
                  onChange={this.handleUpdate(entity.id)}
                />
              }
            </ULItem>
          );
